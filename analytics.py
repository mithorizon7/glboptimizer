...
class AnalyticsManager:
    ...
    def __init__(self):
        # Session managed per-method using context manager
        pass
    ...
    def get_summary_stats(self, days=30):
        ...
        with db_session() as db:
            ...
            total_tasks = db.query(OptimizationTask).filter(
                OptimizationTask.created_at >= cutoff_date
            ).count()
            
            # Successful optimizations
            successful_tasks = db.query(OptimizationTask).filter(
                OptimizationTask.created_at >= cutoff_date,
                OptimizationTask.status == 'completed'
            ).count()
            
            # Total file size savings
            size_savings = db.query(
    ...
    def get_web_game_readiness_stats(self, days=30):
        ...
        with db_session() as db:
            ...
            total_count = db.query(PerformanceMetric).filter(PerformanceMetric.created_at >= cutoff_date).count()
            mobile_count = db.query(PerformanceMetric).filter(
                PerformanceMetric.created_at >= cutoff_date,
                PerformanceMetric.mobile_friendly == True
            ).count()
            web_count = db.query(PerformanceMetric).filter(
                PerformanceMetric.created_at >= cutoff_date,
                PerformanceMetric.web_optimized == True
            ).count()
            streaming_count = db.query(PerformanceMetric).filter(
                PerformanceMetric.created_at >= cutoff_date,
